...
import ru.dins.scalaschool.file_to_map.Models.{FileParsingError, Note, NotesWithInfo}
...
class StringParserTest extends AnyFlatSpec with Matchers {
  ...
  "parse" should "return notes list if string is correct" in {
    ...
    StringParser.parse(s, Config.defaultUserSettings) shouldBe Right(
  ...
  it should "return notes list if string have some formatting troubles" in {
    ...
    StringParser.parse(s, Config.defaultUserSettings.copy(lineDelimiter = Some("'"), infoCol = Some(3))) shouldBe Right(
  ...
  it should "return parsing error if line delimiter incorrect" in {
    ...
    StringParser.parse(s, Config.defaultUserSettings) shouldBe Left(
  ...
  "this parse" should "return parsing error if in row delimiter incorrect" in {
    ...
    StringParser.parse(s, Config.defaultUserSettings) shouldBe Left(FileParsingError("Line #1: Name1,Address1"))
  }
  ...
  it should "return notes list if at least one row parsed successful" in {
    ...
    StringParser.parse(s, Config.defaultUserSettings.copy(infoCol = Some(3))) shouldBe Right(
      NotesWithInfo(List(note1), StringParser.parseWithErrReport(1, 3, "Line #2: Name2;Address2")),
    )
  }
